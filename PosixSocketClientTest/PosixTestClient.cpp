...
void PosixTestClient::reqMarketData(TickerId reqId, const Contract &contract, const IBString &genericTicks, bool snapshot) {m_pClient->reqMktData(reqId, contract, genericTicks, snapshot);}
void PosixTestClient::reqRealTimeBars(TickerId id, const Contract &contract, int barSize, const IBString &whatToShow, bool useRTH){m_pClient->reqRealTimeBars(id, contract, barSize, whatToShow, useRTH);}
